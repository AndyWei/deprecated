...
- (instancetype)initWithTitle:(NSString *)caption delegate:(id<UICustomActionSheetDelegate>)delegate buttonTitles:(NSArray *)titles
{
    if (self = [super init])
    {
        ...
        UITapGestureRecognizer *backgroungTap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(hideAlert)];
...
- (void)showInView:(UIView *)view
{
    ...
    [self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"|-0-[bg]-0-|" options:0 metrics:nil views:mainViews]];

    [self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|-0-[bg]-0-|" options:0 metrics:nil views:mainViews]];

    [self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"|-0-[panel]-0-|" options:0 metrics:nil views:mainViews]];

    [self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:[panel]-0-|" options:0 metrics:nil views:mainViews]];
    ...
    if (_blurredBackground)
    {
        ...
        UIImage *outImage =
            [UIImageEffects imageByApplyingBlurToImage:inImage withRadius:5.0 tintColor:_blurTintColor saturationDeltaFactor:1.2 maskImage:nil];
    ...
    for (NSString *buttonTitle in buttonTitles)
    {
        ...
        if ([buttonTitles indexOfObject:buttonTitle] == 0)
        {
            ...
            [panel addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:[NSString stringWithFormat:@"V:[%@(55)]-10-|", buttonHash]
                                                                          options:0
                                                                          metrics:nil
                                                                            views:views]];
        }
        else
        {
            ...
            NSString *nextButtonHash =
                [NSString stringWithFormat:@"_%lu", (unsigned long)[[buttonTitles objectAtIndex:[buttonTitles indexOfObject:buttonTitle] - 1] hash]];

            if ([buttonTitles indexOfObject:buttonTitle] != [buttonTitles count] - 1)
            {

                [panel addConstraints:[NSLayoutConstraint
                                          constraintsWithVisualFormat:[NSString stringWithFormat:@"V:[%@(55)]-10-[%@]", buttonHash, nextButtonHash]
                                                              options:0
                                                              metrics:nil
                                                                views:views]];
            }
            else
            {
                ...
                NSString *mask = (!_title && !_subtitle) ? @"V:|-10-[%@(55)]-10-[%@]" : @"V:[%@(55)]-10-[%@]";
    ...
    if ([buttonTitles count] > 0)
    {
        ...
        if (_subtitle)
        {
            ...
            NSString *lastButtonHash =
                [NSString stringWithFormat:@"_%lu", (unsigned long)[[buttonTitles objectAtIndex:[buttonTitles count] - 1] hash]];
