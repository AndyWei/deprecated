...
func TestCreatePost(test *testing.T) {
    ...
    resp := httptest.NewRecorder()
    h.Userline(resp, req, userid, username)
    ...
    for _, f := range Friends {
        ...
        resp := httptest.NewRecorder()
        h.Timeline(resp, req, f.fid, f.fname)
        bytes := resp.Body.Bytes()
